...
source = imread("big/training/radcliffe1.jpg")
im = np.reshape(source,(1,227,227,3)).astype(float32)
im = im - im.mean()
grad = sess.run(tf.gradients(layer,x), feed_dict={x:im})[0][0,:,:,:]
grad = np.clip(grad, 0, grad.max()+1)
grad = grad/grad.max()
f,axarr = subplots(1,2)
axarr[0].imshow(source, interpolation='nearest')
axarr[0].axis('off')
