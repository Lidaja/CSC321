...
layer = tf.nn.softmax(tf.matmul(tf.reshape(conv4,[1,13*13*384])/255., W)+b)
...
for a in range(len(act)):
    for p in glob.glob("big/training/"+act[a]+"*"):
        if imread(p).shape == (227,227,3):
            ...
            im = im - im.mean()
            if argmax(sess.run(layer, feed_dict={x:im})) == a:
                correct += 1
            total += 1
            if total%10 == 0:
                print total

print correct/(total*1.00)
        

name = 'butler'
im = np.reshape(imread("big/training/"+name+"1.jpg"),(1,227,227,3))
output = sess.run(layer, feed_dict={x:im})
if argmax(output) == act.index(name):
    print "correct"
else:
    print "incorrect"
print zip(act,output.tolist()[0])