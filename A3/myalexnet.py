...
for a in range(len(act)):
    ...
    for p in glob.glob("big/training/"+act[a]+"*"):
        if imread(p).shape == (227,227,3):
            im = imread(p).astype(float32)
            im = im - im.mean()
            M["train"][a].append(sess.run(conv4, feed_dict = {x:np.resize(im,(1,227,227,3))}))
    ...
    for p in glob.glob("big/testing/"+act[a]+"*"):
        if imread(p).shape == (227,227,3):
            im = imread(p).astype(float32)
            im = im - im.mean()
            M["test"][a].append(sess.run(conv4, feed_dict = {x:np.resize(im,(1,227,227,3))}))
    ...
    for p in glob.glob("big/validating/"+act[a]+"*"):
        if imread(p).shape == (227,227,3):
            im = imread(p).astype(float32)
            im = im - im.mean()
            M["validate"][a].append(sess.run(conv4, feed_dict = {x:np.resize(im,(1,227,227,3))}))

savemat("activations.mat",M)
