...
nhid = 800
W0 = tf.Variable(tf.random_normal([32*32*3,nhid], stddev=0.005))
b0 = tf.Variable(tf.random_normal([nhid], stddev=0.05))

W1 = tf.Variable(tf.random_normal([nhid, 6], stddev=0.005))
b1 = tf.Variable(tf.random_normal([6], stddev=0.005))
...
snapshot["b0"] = sess.run(b0)
snapshot["b1"] = sess.run(b1)
cPickle.dump(snapshot,  open("snapshots1/new_snapshotpart3_"+str(nhid)+"2.pkl", "w"))